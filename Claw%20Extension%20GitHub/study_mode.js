/* CLAW — study_mode.js v24 | Modo Estudo — Smart Router */
(function(){
'use strict';
...
try {
  chrome.storage.sync.get(['claw_study'], d => {
    if (d.claw_study) activate();
  });
} catch(_) {}
...
function showPanel(){
  ...
  panelEl.innerHTML = `
    <div style="display:flex;align-items:center;gap:8px;padding:14px 16px;background:rgba(255,255,255,.03);border-bottom:1px solid rgba(255,255,255,.06);">
      ...
      <span style="margin-left:auto;font-size:10px;color:#52525b;font-weight:500;">v24</span>
...
function callGroq(prompt){
  return new Promise((resolve, reject) => {
    chrome.runtime.sendMessage({
      type: 'GROQ_FETCH',
      payload: { model: MODEL, messages: [{ role: 'user', content: prompt.slice(0, 4000) }], max_tokens: 600, temperature: 0.2, claw_purpose: 'reason' }
    }, res => {
