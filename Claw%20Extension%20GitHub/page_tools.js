...
(function () {
  'use strict';
  const MODEL = 'llama-3.3-70b-versatile'; // 70B: compreensão profunda para resumos
  ...
  chrome.runtime.onMessage.addListener((msg, _, reply) => {
    if (msg.type === 'TOOLS_ON') {
      ...
      /* Se ambos desligados, desmonta */
      if (!SUMMARIZE_ON && !EXPLAIN_ON) { _teardown(); reply({ ok: true }); return false; }
      _setup();
      reply({ ok: true });
    }
  ...
  /* Alt+S — resumir texto selecionado ou pagina inteira */
  ...
  try {
    chrome.storage.sync.get(['claw_summarize', 'claw_explain'], d => {
      SUMMARIZE_ON = !!d.claw_summarize;
      EXPLAIN_ON   = !!d.claw_explain;
  ...
  function _showExplainTip(x, y, txt) {
    ...
    tip.innerHTML = `
      <div style="display:flex;align-items:center;gap:6px;padding:8px 11px;
        border-bottom:1px solid #2e2e34;background:#1e1e22;flex-shrink:0;">
