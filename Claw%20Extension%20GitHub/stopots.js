/* ================================================================
   CLAW — stopots.js v43 Study Assist
   Sugestões de Stop/Adedonha + copiar manualmente
   Desenvolvido por Emanuel Felipe
================================================================ */
(function(){
'use strict';
const MODEL = 'auto'; // Smart Router: vocabulário/criatividade com fallback
...
async function run(letter,pairs){
  ...
  if(allCached){
    ...
    for(let i=0;i<pairs.length;i++) if(cached[i]) markDone(i);
    setState('done','Sugestões do cache prontas — clique para copiar');
    try{ window.ClawStats?.addStop(); }catch{}
    await sleep(9000); busy=false; lastKey='';
  ...
  try{
    ...
    /* ── Anti-repetição: pega últimas respostas usadas por categoria ── */
    const recentKey = 'claw_stop_recent';
    const recentData = await new Promise(r => chrome.storage.local.get([recentKey], d => r(d[recentKey]||{})));
    const avoidLines = expanded.map((cat,i) => {
      const catKey = (letter+'|'+pairs[i].cat).toLowerCase();
      const recent = recentData[catKey] || [];
      return recent.length ? `   (evite: ${recent.slice(-5).join(', ')})` : '';
    });

    const prompt=
      `Você está jogando Stop/Adedonha com amigos brasileiros. Letra da rodada: "${letter}".\n`+
      `\nPara cada categoria abaixo, responda como um brasileiro jovem jogaria: use nomes, marcas, músicas, times, cidades e coisas que qualquer pessoa no Brasil conhece e aceitaria.\n`+
      `\nRegras importantes:\n`+
      `- A resposta DEVE começar com a letra "${letter}" (obrigatório)\n`+
      `- Use respostas populares no Brasil, não palavras obscuras ou estrangeiras desconhecidas\n`+
      `- Para nomes: use nomes comuns no Brasil\n`+
      `- Para times: prefira times brasileiros conhecidos\n`+
      `- Para cidades/estados: prefira brasileiros\n`+
      `- Para músicas/cantores: pode ser nacional ou internacional famoso no Brasil\n`+
      `- Para comidas: comidas que brasileiros comem no dia a dia\n`+
      `- Máximo 3 palavras por resposta\n`+
      `- Se realmente impossível para a letra, use "-"\n`+
      `- MUITO IMPORTANTE: Não repita respostas já usadas (listadas abaixo por categoria)\n\n`+
      expanded.map((c,i)=>`${i+1}. ${c}${avoidLines[i]}`).join('\n')+
      `\n\nResponda SOMENTE JSON sem explicações: {"r":["resp1","resp2",...]}`;

    /* usa background como proxy para garantir que fetch funciona */
    const groqReply = await new Promise((resolve, reject) => {
      chrome.runtime.sendMessage({
        type:'GROQ_FETCH', 
        payload:{model:MODEL,claw_purpose:'general',messages:[{role:'user',content:prompt}],max_tokens:500,temperature:0.7}
      }, res => {
    ...
    setState('done','Sugestões prontas — clique em uma para copiar');

    /* salva cache + anti-repetição */
    try{
      for(let i=0;i<pairs.length;i++){
      ...
      /* anti-repetição: salva resposta no histórico recente por categoria */
      const recentKey = 'claw_stop_recent';
      chrome.storage.local.get([recentKey], d => {
        const recent = d[recentKey] || {};
        for(let i=0;i<pairs.length;i++){
          if(!answers[i]) continue;
          const catKey = (letter+'|'+pairs[i].cat).toLowerCase();
          if(!recent[catKey]) recent[catKey] = [];
          recent[catKey].push(answers[i]);
          if(recent[catKey].length > 20) recent[catKey].shift(); // mantém últimas 20
        }
        // Limita total de chaves
        const keys = Object.keys(recent);
        if(keys.length > 500) delete recent[keys[0]];
        chrome.storage.local.set({ [recentKey]: recent });
      });
    }catch{}

    setState('done','Pronto — copie e preencha manualmente');
...
.cs-ans{color:#d4a85a;font-weight:600;min-width:70px;text-align:right;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;transition:color .25s;cursor:pointer;border-radius:4px;padding:1px 3px;} .cs-ans:hover{background:rgba(212,168,90,.12);color:#f0efed;}
...
function showList(items){
  ...
  el.innerHTML=items.map(({cat,ans},i)=>
    `<div class="cs-row"><span class="cs-cat" title="${escapeHTML(cat)}">${escapeHTML(cat)}</span><span class="cs-ans" id="__csa${i}__" title="Clique para copiar">${escapeHTML(ans)}</span></div>`
  ).join('');
  items.forEach(({ans},i)=>{
    const a=document.getElementById(`__csa${i}__`);
    if(a) a.onclick=()=>copySuggestion(ans);
  });
}
...
function copySuggestion(text){
  ...
  try{ const ta=document.createElement('textarea'); ta.value=val; ta.style.position='fixed'; ta.style.opacity='0'; document.body.appendChild(ta); ta.select(); document.execCommand('copy'); ta.remove(); }catch{}
  setState('done','Copiado: '+val);
}
