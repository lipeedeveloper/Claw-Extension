  ...
  if(msg.type==='AGENT_ACTION'){
    (async()=>{
      try{
        ...
        /* ── Rate limiting ── */
        if(window.ClawRateLimit){
          const check = window.ClawRateLimit.check(msg.action);
          if(!check.allowed){
            const waitSec = (check.waitMs/1000).toFixed(1);
            reply({ok:false,result:`⏳ Rate limit: aguarde ${waitSec}s para "${msg.action}"`});
            return;
          }
        }
